...
public class MD5 {
	...
	public void update(final byte b) {
		update(this.state, new byte[] { b }, 0, 1);
	}

	/**
	 * Update buffer with a single integer (only & 0xff part is used, as a
	 * byte)
	 * 
	 * @param i		Integer value, which is then converted to byte as i & 0xff
	 */
	public void update(final int i) {
		update(this.state, new byte[] { (byte) (i & 0xff) }, 0, 1);
	}
	...
	public void update(final String s) {
		update(this.state, s.getBytes(), 0, s.length());
	}
	 ...
	 */
	public void update(final String s, final String charset_name) throws UnsupportedEncodingException {
		update(this.state, s.getBytes((charset_name == null) ? "ISO8859_1" : charset_name), 0, s.length());
	}
	...
	public byte[] hash() {
		if (finals == null) {
			final MD5State stat = new MD5State(state);

			final int[] count_ints = {(int) (stat.count << 3), (int) (stat.count >> 29)};
			final byte[] bits = encode(count_ints, 8);

			final int index = (int) (stat.count & 0x3f);
			final int padlen = (index < 56) ? (56 - index) : (120 - index);

			update(stat, padding, 0, padlen);
			update(stat, bits, 0, 8);
			finals = stat;
		}
	 ...
	 */
	public String asHex() {
		return Hex.encode(this.hash());
	}

	public static String asHex(final String s) {
		MD5 md5 = new MD5();
		md5.update(s);
		return Hex.encode(md5.hash());
	}
...
//	**************************** Private Methods *****************************
	...
	/**
	 * Updates hash with the byte buffer given (using at maximum length bytes
	 * from that buffer)
	 * 
	 ...
	 * 					buffer.length)
	 */
	private void update(final MD5State stat, final byte[] buffer, final int offset, int length) {
		int	index, partlen, i;
		finals = null;
		...
		if (length >= partlen) {
			...
			if (partlen == 64) {
				...
				System.arraycopy(buffer, offset, stat.buffer, index, partlen);
		...
		if (i < length)
			System.arraycopy(buffer, i + offset, stat.buffer, index, length - i);
	}
